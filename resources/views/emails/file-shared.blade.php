...
<head>
    ...
    <title>File được chia sẻ với bạn</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: white; margin: 0;">📁 File được chia sẻ</h1>
    </div>
    
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
        <p>Xin chào <strong>{{ $sharedWith->name }}</strong>,</p>
        
        <p><strong>{{ $sharedBy->name }}</strong> đã chia sẻ một file với bạn:</p>
        
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #667eea;">
            <h2 style="margin-top: 0; color: #667eea;">{{ $file->original_name }}</h2>
            <p style="margin: 5px 0;"><strong>Kích thước:</strong> {{ $file->formatted_size }}</p>
            <p style="margin: 5px 0;"><strong>Loại:</strong> {{ $file->mime_type }}</p>
            @if($expiresAt)
